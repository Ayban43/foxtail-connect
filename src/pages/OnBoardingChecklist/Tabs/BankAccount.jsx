...
const BankAccount = ({
  ...
  async function handleRemoveClick() {
    ...
    const { error } = await supabase.storage
      .from("summary_files")
      .remove([profitLossFileUrl.replace(supabase.storageUrl + "/object/public/summary_files/", "")]);
  ...
  async function handleFileInputChange(e) {
    ...
    const { data, error } = await supabase.storage
      .from("summary_files")
      .upload("profit_loss/" + uuidv4(), file, { public: true });
  ...
  useEffect(() => {
    if (profitLossFileUrl) {
      const getNumPages = async () => {
        try {
          const pdf = await fetch(profitLossFileUrl);
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold">Bank Account Statements </h2><span className="mb-4 text-gray-600">(Containing all business-related transaction history from the beginning of the last tax year to present.)</span>
      <Options />
      <div
        className="mb-4"
        onDragOver={(e) => {
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 justify-start align-middle">
          <div className="flex bg-slate-200 h-48 w-32 items-center justify-center">Sample File</div>
          <div className="flex bg-slate-200 h-42 w-32 items-center justify-center">Sample File</div>
          <div className="flex bg-slate-200 h-42 w-32 items-center justify-center">Sample File</div>
          <div className="flex bg-slate-200 h-42 w-32 items-center justify-center">Sample File</div>
