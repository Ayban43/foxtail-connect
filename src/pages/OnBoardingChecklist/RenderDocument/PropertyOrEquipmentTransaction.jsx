...
const PropertyOrEquipmentTransaction = ({ userData }) => {
    ...
    const handleOptionChange = async (event) => {
        ...
        const { data, error } = await supabase
            .from('clients')
            .update({ documents_related_to_property_or_equipment_transaction: selectedOptionValue })
    ...
    useEffect(() => {
        const fetchData = async () => {
            const { data, error } = await supabase
                .from('clients')
                .select('documents_related_to_property_or_equipment_transaction')
                .eq('email', userData.email)
                .single();
            ...
            if (error) {
                ...
                setSelectedOption(data.documents_related_to_property_or_equipment_transaction);
            }
        };
    ...
    async function handleFileInputChange(e) {
        ...
        const { data, error } = await supabase.storage
            ...
            .upload(userData.email + '/documents_related_to_property_or_equipment_transaction/' + file.name, file, { public: true });
    ...
    async function fetchFiles() {
        const { data, error } = await supabase.storage
            .from('on_boarding_checklist')
            .list(userData.email + '/documents_related_to_property_or_equipment_transaction/');
    ...
    const handleDeleteFile = async (fileId, fileN) => {
        try {
            const fileName = `${userData.email}/documents_related_to_property_or_equipment_transaction/${fileN}`;
        ...
        <div>
            <div className='p-4'>
                <div className="p-4 bg-orange-100 my-2 rounded-lg">
                    <h1 className="font-bold text-2xl py-2"> Documents Related to Property or Equipment Transactions </h1>
                    <div className="text-gray-600">
                        (Including purchase or sales receipts of any tangible fixed assets)
                    </div>
                </div>
            </div>
                            ...
                            >
                                ...
                                >
                                    {file?.name.match(/\.(jpeg|jpg|gif|png|webp)$/) ? (
                                        <img
                                            className="max-h-52"
                                            src={`https://jpmefbzznkiwbnscdrvn.supabase.co/storage/v1/object/public/on_boarding_checklist/${userData.email + "/documents_related_to_property_or_equipment_transaction/" + file.name}`}
                                            alt={file.name}
                                        />
                                    ) : file?.name.endsWith('.pdf') ? (
                                        <PdfSvg />
...
export default PropertyOrEquipmentTransaction