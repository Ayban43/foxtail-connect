...
const SummaryOfSales = ({ userData }) => {
    ...
    const handleOptionChange = async (event) => {
        ...
        const { data, error } = await supabase
            .from('clients')
            .update({ summary_of_sales_and_cash_check_receipt: selectedOptionValue })
    ...
    useEffect(() => {
        const fetchData = async () => {
            const { data, error } = await supabase
                .from('clients')
                .select('summary_of_sales_and_cash_check_receipt')
                .eq('email', userData.email)
                .single();
            ...
            if (error) {
                ...
                setSelectedOption(data.summary_of_sales_and_cash_check_receipt);
            }
        };
    ...
    async function handleFileInputChange(e) {
        ...
        const { data, error } = await supabase.storage
            ...
            .upload(userData.email + '/summary_of_sales_and_cash_check_receipt/' + file.name, file, { public: true });
    ...
    async function fetchFiles() {
        const { data, error } = await supabase.storage
            .from('on_boarding_checklist')
            .list(userData.email + '/summary_of_sales_and_cash_check_receipt/');
    ...
    const handleDeleteFile = async (fileId, fileN) => {
        try {
            const fileName = `${userData.email}/summary_of_sales_and_cash_check_receipt/${fileN}`;
        ...
        <div>
            <div className='p-4'>
                <div className="p-4 bg-orange-100 my-2 rounded-lg">
                    <h1 className="font-bold text-2xl py-2"> Summary of Sales and Cash/Check Receipt </h1>
                    <div className="text-gray-600">
                        (A complete record of all sales transactions containing informative data such as amount, date of sale, product/service sold, payor, etc.)
                    </div>
                </div>
            </div>
                        ...
                        {documentFiles?.map((file) => (
                            ...
                            >
                                ...
                                <div className="flex justify-center items-center w-full bg-white row-span-5 rounded-lg"
                                    onClick={() => {
                                        const fileUrl = `https://jpmefbzznkiwbnscdrvn.supabase.co/storage/v1/object/public/on_boarding_checklist/${userData.email}/summary_of_sales_and_cash_check_receipt/${file.name}`;
                                ...
                                >
                                    {file?.name.match(/\.(jpeg|jpg|gif|png|webp)$/) ? (
                                        <img
                                            className="max-h-52"
                                            src={`https://jpmefbzznkiwbnscdrvn.supabase.co/storage/v1/object/public/on_boarding_checklist/${userData.email + "/summary_of_sales_and_cash_check_receipt/" + file.name}`}
                                            alt={file.name}
                                        />
                                    ) : file?.name.endsWith('.pdf') ? (
                                        <PdfSvg />
