...
const PayrollData = ({ userData }) => {
    ...
    const handleOptionChange = async (event) => {
        ...
        const { data, error } = await supabase
            .from('clients')
            .update({ payroll_data: selectedOptionValue })
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (error) {
                ...
                setSelectedOption(data.payroll_data);
            }
        };
    ...
    async function handleFileInputChange(e) {
        ...
        const { data, error } = await supabase.storage
            ...
            .upload(userData.email + '/payroll_data/' + file.name, file, { public: true });
    ...
    async function fetchFiles() {
        const { data, error } = await supabase.storage
            .from('on_boarding_checklist')
            .list(userData.email + '/payroll_data/');
    ...
    const handleDeleteFile = async (fileId, fileN) => {
        try {
            const fileName = `${userData.email}/payroll_data/${fileN}`;
        ...
        <div>
            <div className='p-4'>
                <div className="p-4 bg-orange-100 my-2 rounded-lg">
                    <h1 className="font-bold text-2xl py-2"> Payroll Data </h1>
                    <div className="text-gray-600">
                        (Containing employee/contractor headcount, wage amounts, pay periods and dates, payroll tax filing, etc.)
                    </div>
                </div>
            </div>
                        ...
                        {documentFiles?.map((file) => (
                            ...
                            >
                                ...
                                <div className="flex justify-center items-center w-full bg-white row-span-5 rounded-lg transition-all ease-in delay-75 hover:cursor-pointer hover:scale-105"
                                    onClick={() => {
                                        const fileUrl = `https://jpmefbzznkiwbnscdrvn.supabase.co/storage/v1/object/public/on_boarding_checklist/${userData.email}/payroll_data/${file.name}`;
                                ...
                                >
                                    {file?.name.match(/\.(jpeg|jpg|gif|png|webp)$/) ? (
                                        <img
                                            className="max-h-52"
                                            src={`https://jpmefbzznkiwbnscdrvn.supabase.co/storage/v1/object/public/on_boarding_checklist/${userData.email + "/payroll_data/" + file.name}`}
                                            alt={file.name}
                                        />
                                    ) : file?.name.endsWith('.pdf') ? (
                                        <PdfSvg />
