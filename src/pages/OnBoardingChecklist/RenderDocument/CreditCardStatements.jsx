...
const CreditCardStatements = ({ userData }) => {
    ...
    const handleOptionChange = async (event) => {
        ...
        const { data, error } = await supabase
            .from('clients')
            .update({ credit_card_statement: selectedOptionValue })
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (error) {
                ...
                setSelectedOption(data.credit_card_statement);
            }
        };
    ...
    async function handleFileInputChange(e) {
        ...
        const { data, error } = await supabase.storage
            ...
            .upload(userData.email + '/credit_card_statement/' + file.name, file, { public: true });
    ...
    async function fetchFiles() {
        const { data, error } = await supabase.storage
            .from('on_boarding_checklist')
            .list(userData.email + '/credit_card_statement/');
    ...
    const handleDeleteFile = async (fileId, fileN) => {
        try {
            const fileName = `${userData.email}/credit_card_statement/${fileN}`;
        ...
        <div>
            <div className='p-4'>
                <div className="p-4 bg-orange-100 my-2 rounded-lg">
                    <h1 className="font-bold text-2xl py-2"> Credit Card Statements </h1>
                        ...
                        {documentFiles?.map((file) => (
                            ...
                            >
                                ...
                                <div className="flex justify-center items-center w-full bg-white row-span-5 rounded-lg"
                                    onClick={() => {
                                        const fileUrl = `https://jpmefbzznkiwbnscdrvn.supabase.co/storage/v1/object/public/on_boarding_checklist/${userData.email}/credit_card_statement/${file.name}`;
                                ...
                                >
                                    {file?.name.match(/\.(jpeg|jpg|gif|png|webp)$/) ? (
                                        <img
                                            className="max-h-52"
                                            src={`https://jpmefbzznkiwbnscdrvn.supabase.co/storage/v1/object/public/on_boarding_checklist/${userData.email + "/credit_card_statement/" + file.name}`}
                                            alt={file.name}
                                        />
                                    ) : file?.name.endsWith('.pdf') ? (
                                        <PdfSvg />
